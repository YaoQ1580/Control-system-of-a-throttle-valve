	...
	while(1)
	{
		switch(state)
		{
			...
			case 10:
				...
				// 根据RTU协议，WAIT_2T应该要超过1.5个字符发送时间，一个字符是8位，外加起始和终止位，则一个字符为10位，根据波特率位115200，可以算出
			  // 1.5个字符时间为：1/115200 * 10 * 1.5 = 1.3020e-4 s = 0.13ms, 经测试1ms时间比较稳定
				ret = rt_sem_take(&rtu_rx_sem, UPCPU_WAIT_2T); 
				if(ret == RT_EOK) // 如果收到信号量
				{
					rt_device_read(upcpu_rtu, -1, ptr, 1); // 读取缓冲区字符
					ptr ++;
					cnt ++;
				}
				else // 如果没有收到信号量，那么说明一帧数据结束
				{
					...
					if(!rt_memcmp(CRC_code_byte, rx_buffer + cnt - 2, 2)) // 如果生成的校验位与收到的校验位一致
					{
						...
						Process(rx_buffer, cnt, upcpu_rtu);     // 根据mosbus RTU协议来对相应的线圈和寄存器进行读写操作
